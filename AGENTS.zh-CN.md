...
当任务明确匹配时，优先使用对应的 OpenCode skill 或 agent：
...
对于编码、调试、架构、代码审查和验证任务，优先使用对应的开发类 skill 或 agent，而不是临时 improvising。

---

## OpenCode 特定规则

- 不要假设 OpenCode 支持 Claude Code 的 slash commands 或 hooks。
- 当任务匹配时，使用 OpenCode skills 和 agents。
- plugin 只是辅助，不替代清楚的推理。
- 本地文件改动应限制在用户请求的范围内。
- 使用 shell command 时，优先选择容易检查和重复执行的命令。
